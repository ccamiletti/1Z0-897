...
public class AdagesProvider implements Provider<Source> {
	...
	private Source doGet(MessageContext mctx) {
		...
		// Get a specified Adage.
			...
			adage.setWords("this is a word....");
			adage.setWordCount(id);
			if (adage == null)
	...
	private Source doPut(Source request) {
		...
		try {
			...
			// Find and edit.
		Adage adage = null; //Adages.find(id);
		if (adage == null)
			throw new HTTPException(404); // not found
		adage.setWords(parts[0]);
		String msg = "Adage " + /*adage.getId()*/ 1 + " has been updated.";
		return toSource(toXml(msg));
	}
	...
	private StreamSource adages2Xml() {
		String str = toXml(/*Adages.getListAsArray()*/ null);
		return toSource(str);
	}
